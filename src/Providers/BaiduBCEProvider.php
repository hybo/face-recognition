...
class BaiduBCEProvider extends AbstractProvider implements BaiduBCEInterface
{
    ...
    {
        $accessToken = $this->cache()->get('baidubce.access_token');
        if (empty($accessToken)) {
            ...
            if (isset($res['access_token']) && isset($res['expires_in'])) {
                ...
                $this->cache()->set('baidubce.access_token', $accessToken, $res['expires_in']);
            } else {
    ...
    private function request($url, $data)
    {
        ...
        if ($res['error_code'] > 0) {
            ...
            if ($res['error_code'] == 110) {
                $this->cache()->delete('baidubce.access_token');
